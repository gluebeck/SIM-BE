...
N=1000
MaxAge = 65      # pre-initiations and initiation events only up to this age
Tdeath = 80
BEonset.max = 60 # should be Tdeath unless conditioning on BE presence at a given screening age

# screening times (array with integer years)
BEscreenings = seq(60,65,.5)
firstScreen = min(BEscreenings)
lastScreen = max(BEscreenings)

# constant time increment for computing distributions
...
source('BE_MSCE1_initiation_2.R')
...
info = system.time(
  for (n in 1:N){
    ...
    # BEage = firstScreen-BEonset
    
    #############################################################################
    # define data frame that includes all intended screens for this individual 
    ns = length(BEscreenings)
    datf = data.frame(ID=rep(n,ns),age=BEscreenings,ageClinicalEAC=rep(Inf,ns),
                      BEonset=rep(BEonset,ns),
                      BE = rep(FALSE,ns),
                      NDBE = rep(FALSE,ns),
                      LGD=rep(FALSE,ns),lgd=rep(FALSE,ns),
                      HGD=rep(FALSE,ns),hgd=rep(FALSE,ns),
                      IMC=rep(FALSE,ns),
                      InSitu=rep(FALSE,ns),
                      EAC=rep(FALSE,ns),
                      EACpreScreen = rep(FALSE,ns),
                      EACpostScreen=rep(FALSE,ns))
    ...
    if(firstScreen < Inf) {
      ...
      time.to.firstScreen = firstScreen-(BEonset+s.malignant)
      ...
      # if(n==1) {
      #   Ymalign = Y.initiated.malignant
      #   Smalign = s.initiated.malignant}
      ...
      while(runT <= tmax) { #} & cond.stop == FALSE) {
        ...
        if(age%in%BEscreenings==TRUE) {
          ...
          if(IndicatorFirstScreen == FALSE) {
            ...
            # Malign$LGD[cond2][cond3] = FALSE
            # Malign$HGD[cond2][cond3] = TRUE
